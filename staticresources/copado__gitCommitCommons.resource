...
(function (app) {
    ...
    app.refreshRecentChanges = function (orgId) {
        ...
        core.GitCommit_GridController.retrieveRecentChanges(orgId, function (result, event) {
            if (event.status) {
                ...
                if (sourceMembers && sourceMembers.length) {
                    ...
                    for (const member of sourceMembers) {
                        ...
                        const mdtWrapper = {
                            t: member.MemberType,
                            n: decodeURI(member.MemberName.replace(/%20/g," ")),
                    ...
                    // brackets needs to be encoded since sfdc gives encoded chars
                    for(var i=0; i<allMdt.length; i++){
                        allMdt[i].n = (allMdt[i].n).replace("(","%28").replace(")","%29").replace("[","%5B").replace("]","%5D");
                    }
            ...
            if (app.grid.isTypeFilterable && app.returnParameter('MetadataPreselectionId')) {
                setWithoutRebinding(app.userStoryMD, selectedMdt);
                app.grid.reloadSelectionsJSON(app.userStoryMD, 1);
            }
            app.grid.render();
            app.grid.loadMetaData();
